...
def updated_dur_fun(alpha, tau, c, M, K, L, Nk, rate_exp, rate_su, epsilon, pui, time_duration):
    ...
    for rate_pu in pui:
        ...
        # Initilize b randomly between 0 and 1
        b = {su: {suj: np.random.random() for suj in neighbors[su]} for su in SUs}
        ...
        for t in time:
            ...
            for su in SUs:
                for c in channels:
                    for suj in neighbors[su]:
                        if s[su][c][suj] != 0 or s[su][c][suj] != 1 and tj[su][c][suj] != t:
                            s[su][c][suj] = b[su][suj] - np.exp(tj[su][c][suj] - t)
